...
A fork of `gno.land/p/nt/avl/v0` from [gnolang/gno at `f3d5a5d13`](https://github.com/gnolang/gno/tree/f3d5a5d13c09e19fadcb836e8473eef3177ed429/examples/gno.land/p/nt/avl/v0),
byte-identical apart from the module and import paths, published so our realms
keep the two-value `Get` after gno made it single-value. `doc.gno`, `node.gno`
and `tree.gno` match what is deployed at `gno.land/p/samcrew/avl` on gnoland-1.
Like its source, it is licensed under the GNO Network General Public License
([`LICENSE`](LICENSE)), not the MIT license of the rest of this repository.
